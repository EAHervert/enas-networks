...
from utilities.functions import SSIM, generate_loggers, drop_weights, display_time
...
parser = argparse.ArgumentParser(
    prog='Train_CNN_{date}'.format(date=d1),
    description='Trains Vanilla CNN',
)
...
parser.add_argument('--model_path', default='CNN_SIDD.pth', type=str)  # Model path
parser.add_argument('--training_path_csv', default='sidd_np_instances_064_0016.csv', type=str)  # Training path
...
def main():
    ...
    config_path = dir_current + '/configs/config_dncnn.json'
    ...
    if args.load_model:
        state_dict_dncnn = torch.load(dir_current + args.model_path, map_location=device)
        if args.drop > 0:
            state_dict_dncnn = drop_weights(state_dict_dncnn, p=args.drop, device=device)

        dncnn.load_state_dict(state_dict_dncnn)
    ...
    optimizer = torch.optim.Adam(dncnn.parameters(),
    ...
    for epoch in range(args.epochs):
        for i_batch, sample_batch in enumerate(dataloader_training):
            ...
            eta = dncnn(x.to(device))  # Estimate the noise
            t = sample_batch['GT']

            loss_value = loss(eta, x - t)  # Difference between denoised and target
            ...
            with torch.no_grad():
                ...
                psnr_batch.update(PSNR(x - eta, t).item())
                psnr_original_batch.update(PSNR(x, t).item())

            # Backpropagate to train model
            optimizer.zero_grad()
            nn.utils.clip_grad_norm_(dncnn.parameters(), config['Training']['Child_Grad_Bound'])
            loss_value.backward()
            optimizer.step()

            if i_batch % 100 == 0:
                Display_Loss = "Loss_DNCNN: %.6f" % loss_batch.val + "\tLoss_Original: %.6f" % loss_original_batch.val
                Display_SSIM = "SSIM_DNCNN: %.6f" % ssim_batch.val + "\tSSIM_Original: %.6f" % ssim_original_batch.val
                Display_PSNR = "PSNR_DNCNN: %.6f" % psnr_batch.val + "\tPSNR_Original: %.6f" % psnr_original_batch.val
        ...
        Display_Loss = "Loss_DNCNN: %.6f" % loss_batch.avg + "\tLoss_Original: %.6f" % loss_original_batch.avg
        Display_SSIM = "SSIM_DNCNN: %.6f" % ssim_batch.avg + "\tSSIM_Original: %.6f" % ssim_original_batch.avg
        Display_PSNR = "PSNR_DNCNN: %.6f" % psnr_batch.avg + "\tPSNR_Original: %.6f" % psnr_original_batch.avg
        ...
        for i_validation, validation_batch in enumerate(dataloader_validation):
            with torch.no_grad():
                ...
                loss_batch_val.update(loss(x_v - eta_v, t_v).item())
                loss_original_batch_val.update(loss(x_v, t_v).item())
                ssim_batch_val.update(SSIM(x_v - eta_v, t_v).item())
                ssim_original_batch_val.update(SSIM(x_v, t_v).item())
                psnr_batch_val.update(PSNR(x_v - eta_v, t_v).item())
                psnr_original_batch_val.update(PSNR(x_v, t_v).item())

            # Free up space in GPU
            del x_v, eta_v, t_v
            torch.cuda.empty_cache()

        Display_Loss = "Loss_DNCNN: %.6f" % loss_batch_val.avg + "\tLoss_Original: %.6f" % loss_original_batch_val.avg
        Display_SSIM = "SSIM_DNCNN: %.6f" % ssim_batch_val.avg + "\tSSIM_Original: %.6f" % ssim_original_batch_val.avg
        Display_PSNR = "PSNR_DNCNN: %.6f" % psnr_batch_val.avg + "\tPSNR_Original: %.6f" % psnr_original_batch_val.avg
        ...
        Legend = ['DNCNN_Train', 'Orig_Train', 'DNCNN_Val', 'Orig_Val']
        ...
        #     model_path_0 = (dir_current +
        #                     '/models/{date}_dncnn_{noise}_{name}_{epoch}.pth'.format(date=d1,
        #                                                                              epoch=epoch,
        #                                                                              name=args.name,
        #                                                                              noise=args.noise))
        #     torch.save(dncnn.state_dict(), model_path_0)
        #     # modify weights to avoid overfitting
        #     if args.clip_weights:
        #         state_dict_dncnn = clip_weights(dncnn.state_dict(), k=3, device=device)
        #         dncnn.load_state_dict(state_dict_dncnn)
    ...
    # Save final model
    model_path_0 = dir_current + '/models/{date}_dncnn_{noise}_{name}.pth'.format(date=d1,
                                                                                  name=args.name,
                                                                                  noise=args.noise)
    torch.save(dncnn.state_dict(), model_path_0)
