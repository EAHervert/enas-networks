...
# Hyperparameters
config_path = os.getcwd() + '/configs/config_sidd_e.json'
...
vis_window = {'EDHDN_SSIM': None, 'EDHDN_PSNR': None}

# Training Optimization and Scheduling:
optimizer = torch.optim.Adam(e_dhdn.parameters(), config['Training']['Learning_Rate'])
...
for epoch in range(config['Training']['Epochs']):
    for i_batch, sample_batch in enumerate(dataloader_sidd_training):
        ...
        Display_Loss = "Loss_EDHDN: %.6f" % loss_batch.val + "\tLoss_Original: %.6f" % loss_original_batch.val
        Display_SSIM = "SSIM_EDHDN: %.6f" % ssim_batch.val + "\tSSIM_Original: %.6f" % ssim_original_batch.val
        Display_PSNR = "PSNR_EDHDN: %.6f" % psnr_batch.val + "\tPSNR_Original: %.6f" % psnr_original_batch.val

        if i_batch % 10 == 0:
            print("Training Data for Epoch: ", epoch, "Image Batch: ", i_batch)
            print(Display_Loss + '\n' + Display_SSIM + '\n' + Display_PSNR)

    Display_Loss = "Loss_EDHDN: %.6f" % loss_batch.avg + "\tLoss_Original: %.6f" % loss_original_batch.avg
    Display_SSIM = "SSIM_EDHDN: %.6f" % ssim_batch.avg + "\tSSIM_Original: %.6f" % ssim_original_batch.avg
    Display_PSNR = "PSNR_EDHDN: %.6f" % psnr_batch.avg + "\tPSNR_Original: %.6f" % psnr_original_batch.avg
    ...
    Display_Loss = "Loss_EDHDN: %.6f" % loss_batch_val.val + \
                   "\tLoss_Original: %.6f" % loss_original_batch_val.val
    Display_SSIM = "SSIM_EDHDN: %.6f" % ssim_batch_val.avg + \
                   "\tSSIM_Original: %.6f" % ssim_original_batch_val.avg
    Display_PSNR = "PSNR_EDHDN: %.6f" % psnr_batch_val.avg + \
    ...
    vis_window['EDHDN_SSIM'] = vis.line(
        ...
        win=vis_window['EDHDN_SSIM'],
        opts=dict(title='EDHDN_SSIM', xlabel='Epoch', ylabel='SSIM', legend=Legend),
        update='append' if epoch > 0 else None)

    vis_window['EDHDN_PSNR'] = vis.line(
        X=np.column_stack([epoch] * 4),
        Y=np.column_stack([psnr_batch.avg, psnr_batch_val.avg,
                           psnr_original_batch.avg, psnr_original_batch_val.avg]),
        win=vis_window['EDHDN_PSNR'],
        opts=dict(title='EDHDN_PSNR', xlabel='Epoch', ylabel='PSNR', legend=Legend),
        update='append' if epoch > 0 else None)
...
model_path = os.getcwd() + '/models/{date}_sidd_e_dhdn.pth'.format(date=d1)
torch.save(e_dhdn.state_dict(), model_path)
