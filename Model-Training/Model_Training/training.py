...
def train_loop(epoch, config, batches, model, architecture, device, Loss, MSE, loss_meter, loss_meter_original,
    ...
    for batch in batches:
        ...
        t0 = time.time()
        index += 1
        # size_crop, number_crops
        if mode == 'Training':
            loader = load_dataset_images(batch, number_crops=config['Training']['Train_Size'])
        else:
            loader = load_dataset_images(batch, size_crop=256, number_crops=config['Training']['Train_Size'])
