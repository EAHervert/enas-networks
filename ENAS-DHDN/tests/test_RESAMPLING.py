...
from ENAS_DHDN import RESAMPLING

CHANNELS = 16
ALPHAS = [0.1, 0.7, 0.2]


def test_downsampling_enas(size):
    x = torch.randn(1, CHANNELS, size[0], size[1])
    target = [1, CHANNELS * 2, size[0] // 2, size[1] // 2]
    enas_down = RESAMPLING._down_ENAS(channel_in=CHANNELS)

    bool_vals = []
    for i in range(3):
        bool_vals.append(list(enas_down(x, int_=i).shape) == target)
...
def test_downsampling_fixed(size):
    ...
    for i in range(3):
        ...
        bool_vals.append(list(fixed_down(x, int_=None).shape) == target)
...
def test_upsampling_enas(size):
    x = torch.randn(1, CHANNELS, size[0], size[1])
    target = [1, CHANNELS // 4, size[0] * 2, size[1] * 2]
    enas_up = RESAMPLING._up_ENAS(channel_in=CHANNELS)

    bool_vals = []
    for i in range(3):
        bool_vals.append(list(enas_up(x, int_=i).shape) == target)
...
def test_upsampling_fixed(size):
    ...
    for i in range(3):
        ...
        bool_vals.append(list(fixed_up(x, int_=None).shape) == target)
...
class test_RESAMPLING(unittest.TestCase):
    def test_sampling(self):
        assert test_downsampling_enas(size=[64, 64])
        assert test_downsampling_fixed(size=[64, 64])
        assert test_downsampling(size=[64, 64])
        assert test_upsampling_enas(size=[64, 64])
